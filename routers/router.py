...
from routers.return_scheme import BaseResponse, DecryptResponse, EncryptResponse
from utils.logger import Logger
from utils.rsa.rsa_class import RSAUtil
from routers.input_scheme import CreateAndUpdataAuthorizeMessage, EcryptMessage
from apps.sql.sqlite_database import SessionLocal, engine
from apps.sql.sqlite_curd import Authorize, Session
import apps.sql.sqlite_models as models
from apps.rsa_record import add_authorize_time
...
def get_authoize(response: Response, UUID: str = Path(..., title="the authoize id"), db: Session = Depends(get_db)):
    try:
        auth = Authorize(db)
...
def create_new_authoize(request_data: CreateAndUpdataAuthorizeMessage, response: Response, db: Session = Depends(get_db)):
    try:
...
def updata_new_authoize(request_data: CreateAndUpdataAuthorizeMessage, response: Response, db: Session = Depends(get_db)):
    try:
...
def delete_authoize(response: Response, db: Session = Depends(get_db), UUID: str = Path(..., title="the authoize id")):
    try:
        auth = Authorize(db)
